...
| **Group B — 法人籌碼** | 16 | 外資/投信/自營商淨買超（絕對值+佔比）、融資餘額、融券餘額、KD(9,3)、OBV、成交量比 |
| **Group C — 基本面** | 10 | 月營收 YoY、EPS、本益比、股價淨值比、ROE、市值、殖利率、自由現金流 |
| **Group D — 宏觀環境** | 8 | TAIEX / SPX / 黃金 報酬率、VIX、美元指數、台幣匯率、PMI、聯邦基準利率 |

訓練資料時間範圍：**2012–2024**，約 3,000 支台股 × 12 年歷史。
...
輸入：(N_stocks, 252 天, 46 特徵)
  ...
  ↓ Mamba SSM × 8 blocks
      d_model=256, d_state=32，線性複雜度時序建模
  ↓ GATv2（知識圖譜引導截面互動）
      ~43,000 條邊，2,890 個節點
  ↓ Linear (256 → 3)
...
## 訊號掃描器邏輯（V6.1，scan_version 1.2）

### 大盤環境判斷

| 條件 | 模式 | 入場門檻 | 觀察清單 |
|------|------|---------|---------|
| TWII > 60 日均線 | 正常市場 | 加權評分 ≥ 55 分 | 評分 1–54 分 |
| TWII < 60 日均線 | 保守模式 | 加權評分 ≥ 70 分 | 評分 1–69 分 |

### 入場條件（加權評分制，滿分 100 分）

| # | 條件 | 分數 | 判斷邏輯 |
|---|------|------|---------|
| 1 | 排名穩定性 | 30 分 | Top 10 連續 ≥2 天 **或** Top 50 連續 ≥3 天 |
| 2 | 高信心度 | 25 分 | Uncertainty < 當日全市場 Q30 分位數（MC-Dropout） |
| 3 | 機構連續淨買 | 25 分 | 外資/投信 連續 ≥2 天淨買入 |
| 4 | 相對低點 | 20 分 | RSI(14) < 40 **或** 當前價 < 20 日均線 |
...
- 最多同時持有 **15 檔**，有空位才建新倉
- 按 `suggested_weight`（Kelly 比例）分配資金
- 單股上限 **10%**，同一產業不超過 **30%** 總部位
- 不足 5 檔時輸出警告
...
- 支援型態（均為多方）：
  - **W底**：兩低點差 < 5%，頸線突破 > 3%
  - **彈簧型W底**：第二低 0.5%–2% 低於第一，快速反彈
  - **頭肩底**：三底，中間最低，兩肩差 < 10%
  - **三角收斂**：低高收斂，至少 4 個觸點
- 型態評分系統（100 分）：型態強度 40 分 + 成交量確認 30 分 + 位置 20 分 + RSI 動能 10 分；Alpha 加成：Top 200 +10 / Top 300 +5
...
│   │   ├── backtest/engine.py   ← 回測引擎（投資模擬機器人用）
