...
Phase 3B-1 source canonicalization is in progress on WSL development main. The V7 status UI/API is committed but not deployed, the E1-matching environment identity is preserved, and 41 early V7 experimental source files are preserved in place with hashes and dated lineage at `research/v7/historical-local-source-register-20260922.md`. Fourteen local research/task notes were preserved verbatim in Git, then marked as dated historical records; `research/v7/current-state.md` explicitly defers to this canonical state file. Neither the early source nor old notes replace corrected E5/E1 branch authority. Branch integration remains open; no worktree or large artifact has been removed.
