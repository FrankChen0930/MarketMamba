...
> ## 🔴 2026-08-04 23:30 更新：**這次跑失敗了，下面的「現況」已作廢**
>
> **h10 在 epoch 3/10 中斷、h20 從未開始。**
>
> **根因：WSL2 VM 在 23:23 自行重啟**（證據：`/tmp` 全清、PID 計數器從 3112 歸零到 382、
> systemd 目錄時間戳全為 23:23），連帶殺掉 `colab new` 派生的 **keep-alive daemon**，
> Colab 隨即回收 A100。
>
> **→ 這是「用 WSL 的 Colab CLI 跑整夜」的結構性弱點**：
> keep-alive 依附在 WSL 程序上，WSL 一重啟就全毀，而**訓練本身沒有任何錯**。
> 本專案早有前例（CLAUDE.md 記載 6/10、6/2 的推論卡死，懷疑 WSL2 冷啟動／睡眠喚醒）。
>
> **保住的東西**（Drive，23:22 寫入）：
> `status_short_H_h10.json`（epoch 3、best_val_ic_5d **0.1073**、仍在上升）
> 與 `v6_short_H_h10.pt`。**但 3/10 epoch 不足以下結論，這一組要重跑。**
>
> **⚠️ 善後**：`colab sessions` 仍列出 `[?] gpu-a100-...` 孤兒（本機紀錄已被
> auto-prune 清掉 → CLI 停不掉）。**必須到 Colab 網頁版 → 管理工作階段 → 終止**，
> 否則可能持續計費。
>
> **重跑建議**：改用 **Colab 網頁版**（瀏覽器分頁自己維持 keep-alive、不依賴 WSL），
> 或先查清楚 WSL 為何重啟再用 CLI。CLI 本身已驗證可用（CPU/T4/A100 全通），
> 問題不在 CLI，在**它的 keep-alive 掛在一個會突然消失的宿主上**。
>
> ---
>
> **給接手的人／下一個 session**：以下是原本的交接內容，**執行現況已作廢**，
> 但「常用指令」「跑完之後要做什麼」「實驗設定」「CLI 的坑」「從零重建」
> 這幾節仍然有效、可直接用。
