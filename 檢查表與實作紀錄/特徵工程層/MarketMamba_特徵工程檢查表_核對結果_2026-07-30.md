...
| C3 | 死旗標處理依**全量**結果定案並同步 `INPUT_DIM` | **✅ 已定案（F5，2026-07-30）：`INPUT_DIM`=59，旗標全不採用** |
...
| G2 | 死旗標最終取捨已依全量定案並反映到 `INPUT_DIM` | **✅ `INPUT_DIM`=59**（GBDT 20d −0.0060 t=−2.93、gain 僅 0.32%；協定 §9.4）|
| G3 | 集團表縮減版本已定案並記錄理由 | **✅** |
| G4 | R0–R5 六組對照已執行，只有正貢獻才帶進重訓 | **✅ 共 14 級已執行**（含 fund_v2 四項子修正拆解與 GBDT 裁決）；結論：無任何正貢獻達門檻，僅保留兩項正確性修正。協定 §9 |
...
### ✅ F5 已跑完，兩項規格於 2026-07-30 定案（詳見協定 §9.4）
6. **C3 / G2**：`INPUT_DIM` = **59** —— 可得性旗標**不採用**。全量下 7 個旗標都不是
   死常數（推翻 40 支小樣本的判定），但 GBDT 給的答案是 5d 無效應、**20d 顯著扣
   0.0060（t=−2.93）**、gain 佔比合計僅 0.32%（307 維均分為 0.326%）。原本最主要的
   動機（9xxx 語意翻轉）已用回補在源頭修好；且**起點移到 2013 與旗標功能高度重疊**。
7. **G4 / R4**：`NEUTRALIZE` = **"none"** —— 四個測量方向全正（最大 +0.0068）、
   去極端日後更強，但 t 值 0.76–1.42 過不了 |t|≥2；而中性化在推論路徑多一個必須
   同步的步驟（訓練/推論不一致是本專案踩過兩次的坑）。留到 F6 用 Mamba 複驗。
   `industry_mktcap` 未量。

**→ 特徵工程層至此真正凍結。** 剩下的都是 F6（Colab/GPU）：GAT 三組消融、
Group D 消融、中性化用 Mamba 複驗。
