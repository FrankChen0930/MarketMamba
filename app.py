...
with st.sidebar:
    ...
    # 🌟 新增：傳統型態學雷達分頁按鈕
    if st.button("📐 傳統型態學雷達", use_container_width=True, type="primary" if st.session_state['current_page'] == "📐 傳統型態學雷達" else "secondary"):
        st.session_state['current_page'] = "📐 傳統型態學雷達"
        st.rerun()
...
# ------------------------------------------
# 頁面 2: 個股軌跡透視 (V5.0 Alpha 軌跡版) + 修復 N/A 問題
# ------------------------------------------
elif page == "📈 個股軌跡透視":
            ...
            try:
                ...
                if not hist.empty:
                    ...
                    try:
                        ...
                        # 確保真的有抓到資料，如果沒抓到市值或價格，換下一個後綴試試
                        if 'marketCap' not in info_data and 'regularMarketPrice' not in info_data:
                            continue 
                    except:
                        pass
    ...
    if hist_df is None:
        ...
        mc_str = f"{mc_val / 100000000:,.2f}" if isinstance(mc_val, (int, float)) else "N/A" # 轉換為「億」
        ...
        fig.add_trace(go.Scatter(
            x=[future_dates[0], future_dates[-1]], y=[0, 0], mode='lines', 
            name='TWII 大盤基準線 (0%)', line=dict(color='white', width=2, dash='dash')
        ))
        
        line_color = '#ff4b4b' if alpha_trajectory_pct[-1] > 0 else '#00fa9a'
        fig.add_trace(go.Scatter(
            x=future_dates, y=alpha_trajectory_pct, mode='lines+markers', 
            name=f'{target_ticker} 預測 Alpha', line=dict(color=line_color, width=4), marker=dict(size=6)
        ))
        ...
        fig.add_trace(go.Scatter(
            x=list(future_dates) + list(future_dates)[::-1], y=list(upper_bound) + list(lower_bound)[::-1], 
            fill='toself', fillcolor='rgba(255, 255, 255, 0.1)', line=dict(color='rgba(255,255,255,0)'), 
            hoverinfo="skip", name='模型信心區間'
        ))

        fig.update_layout(
            title=f"<b>{format_ticker(target_ticker)} 未來 30 天超額報酬預測</b>",
            yaxis_title="累積 Alpha (%)", xaxis_title="未來交易日",
...
# ------------------------------------------
    ...
    st.subheader("📐 傳統型態學掃描雷達 (技術分析完全體)")
    st.write("結合多重時間框架與等幅測距，全自動掃描全市場，精準捕捉發動前夕的高勝率經典型態。")
    st.divider()

    if df_patterns.empty:
        st.success("🛡️ **今日市場結構混亂，雷達未偵測到高勝率經典型態，建議空手觀望，保護資金。**")
        st.info("量化交易金律：「沒有訊號就是最好的訊號」。當市場出現 V 轉或無序震盪時，雷達會自動啟動最高級別防護機制，直到完美教科書型態再次浮現。")
    else:
        st.write("🎯 **今日偵測到符合結構的關鍵型態股：**")
        
        display_df = df_patterns.copy()
        display_df['股票標的'] = display_df['Stock_ID'].apply(format_ticker)
        
        # 整理欲顯示的欄位
        cols_to_show = ['Pattern', '股票標的', 'Scale', 'Current_Price', 'Target_Price', 'Exp_Return(%)']
        ...
        styled_df = display_df.style.applymap(color_returns, subset=['預期報酬空間(%)']) \
                                    .format({
                                        "最新收盤價": "{:.2f}",
                                        "目標滿足價": "{:.2f}",
                                        "預期報酬空間(%)": "{:.2f}%"
...
# ------------------------------------------
    ...
    if ledger["holdings"]:
        ...
        for t, d in ledger["holdings"].items():
            ...
            profit_pct = ((current_p - cost_val) / cost_val * 100) if cost_val > 0 else 0
    ...
    if len(ledger["history"]) > 0:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=hist_df['date'], y=hist_df['equity'], mode='lines+markers', line=dict(color='#ff4b4b', width=3)))
