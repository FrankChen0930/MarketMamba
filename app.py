...
st.title("🐍 MarketMamba V5.5: 終極量化決策中心")
st.markdown("基於 Mamba SSM + KG-Enhanced GAT + 雙軌 FinBERT 情緒引擎，預測 30 天超額報酬 (Alpha) 軌跡。")
with st.expander("📖 MarketMamba V5.5 核心指標與看盤指南 (新手必看)", expanded=False):
    st.markdown("""
    ### 🎯 什麼是 Alpha (超額報酬)？
    V5.5 預測的不是「股價會漲到多少」，而是預測這檔股票**「能贏過大盤多少」**。
    ...
    * **資金佔比 (Kelly Weight)**：基於凱利公式計算出的建議押注比例，最高 20%。
    ...
    ### 📰 V5.5 新增：消息面情緒引擎
    * **雙軌 FinBERT**：英文(ProsusAI)+中文(chinese-finbert)雙模型，即時分析全球財經新聞。
    * **情緒指標**：市場情緒、地緣政治風險、個股消息面熱度。
    * **Knowledge Graph GAT**：結合台股產業分類的知識圖譜，強化供應鏈因果推理。
    """)
...
def load_sentiment_summary():
    try:
        url = "https://raw.githubusercontent.com/FrankChen0930/MarketMamba/main/sentiment_summary.json"
...
sentiment_data, sentiment_available = load_sentiment_summary()

with st.sidebar:
    st.header("📌 功能選單")
    if data_loaded:
        st.success("✅ V5.5 雲端數據庫已連線")
    ...
    if sentiment_available:
        ...
        st.markdown("**📰 消息面快照**")
        cols = st.columns(3)
        sm_us = sentiment_data.get('market_us', 0)
        sm_tw = sentiment_data.get('market_tw', 0)
        sm_geo = sentiment_data.get('geopolitical', 0)
        cols[0].metric("🇺🇸 美", f"{sm_us:+.2f}", delta_color="normal")
        cols[1].metric("🇹🇼 台", f"{sm_tw:+.2f}", delta_color="normal")
        cols[2].metric("🌍 地緣", f"{sm_geo:+.2f}", delta_color="inverse")
...
# ------------------------------------------
    ...
    if hist_df is None or hist_df.empty:
        ...
        st.markdown("#### 📅 V5.5 Mamba 預期超額報酬 (Alpha) 軌跡")
