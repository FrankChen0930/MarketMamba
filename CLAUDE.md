...
- [ ] **WSL：補 GRU 分數**（`result/gru_5d.pt`，Windows 端無 torch）→ 補齊四階對照
- [ ] **確認 v1.1 修訂提案**（規格 §8）：A 改「貼近實際操作」headline 為 20 日再平衡（我原本定成每日是設計失誤）；B 新增「分數平滑」維度。兩者都是看到數字後才提的 → 採用後所有模型要在 v1.1 下重跑，新舊數字不得混用
- [ ] **Colab：跑 Group D 消融**——**排在 GAT 三組跑完之後**（2026-08-01 使用者指示「先放著，等現在的跑完再說」）：`run_groupd_ablation(df, drive_dir=...)`，只需訓練 `no_macro` 一組約 3.6 小時。記得先清 `sys.modules` 快取再 import（§5 第 1 個坑）
- [ ] **③ macro 接進 `run_daily_update`**（regime 閘門 10 天新鮮度即將到期）——**但 V6.1 若退役就不必做**；也應等 Group D 消融證實有貢獻再做
