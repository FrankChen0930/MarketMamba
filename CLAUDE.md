...
> 最後更新：2026-08-05 深夜（**V6.2 推論端＋組合層＋前端全部接完並驗證通過，週一 08-10 上線**。四項關鍵發現：① **接線正確**——Group A/B 逐位元相同、582 天裡 569 天過判準 ② **資料修正對模型的影響是雜訊**（年化 38.0→38.1%、IC 0.1139→0.1139）→ **原本規劃的「重建矩陣＋全部重訓」失去實證支撐、已取消** ③ **margin 不是 T+1**，是我們 19:35 抓太早（21:11 實測有當日資料）→ 改排程即可、不需 `shift(1)` ④ **EPS 截斷是局部缺陷不是全歷史**。**開工先讀本檔「下一步」的 ▶ 區塊**）
>
> 上一版：2026-08-05（V6.2 規格定案：`v2_kg_nomacro` 的 `5d/20`，不需重訓；量到組合層雜訊底線 ±6pp、decile spread 才是比較模型的正確量尺、GRU 排名第 2→第 4。交接單 `docs/session-handoff-2026-08-05.md`）
>
...
### 最近完成
- **★★★ V6.2 上線鏈路全部接完並驗證通過（2026-08-05 深夜）——週一 08-10 上線**。新增五支程式（全部純附加，V6.1／雙模型一個字未動）：
  - **`V6/run_v62_inference.py`**：補齊與 `run_dual_inference` 的**五個**落差。前四個是先前列的（`fundamentals_v2=True`／v2 圖／Group D 輸入端歸零／新 checkpoint），**第五個是這次查程式才發現的：宇宙過濾**——`run_dual_inference` 完全沒有（`run_daily_inference` 有，在 `_sanitize`），而 base matrix 走 `filter_tradable_universe`。ETF 與興櫃留在橫斷面裡會改變 `clean_and_scale` 的 winsorize／z-score **分母** → 59 維每一維都會偏、且不報錯
  - **兩個照抄回測、不可自由發揮的口徑**：① 分數＝`model.eval()` **單次前向**，不是 MC-Dropout 平均（+38.0% 那組是 eval 算的，兩者不相等；MC 只當顯示用的不確定性、`--mc` 才算、不參與排序）② 排序用 **raw score** 不用 `SQ=score/unc`（SQ 是雙模型那條線的設計，混用會讓實盤與回測不同義）
  - **★ 驗證（比原設計強得多）**：原本只檢查一天，實際跑了整窗 582 天。**Group A/B 逐位元相同（34/35 欄 `max|Δ|=0.0000`）→ 接線正確**；**569/582 天（97.8%）同時通過 ρ≥0.95 與 Top50 重疊≥40**
  - **⚠️ 我的驗證設計失誤**：第一次挑 **2026-06-02** 當測試日 → ρ=0.9398 **未過**。那是**窗的最後一天**，而 MOPS 補的 2026Q1 財報 `available_from` = 季末+45 天 = **2026-05-15** → 我保證挑中受新資料影響最大的那 13 天之一。逐日分布：**2026-05-15 之前平均 ρ=0.9941、之後 0.9324**，分界線精確吻合。**單日驗證挑在窗緣＝專挑最差的一天**
  - **`V6/v62_portfolio.py`**：組合層狀態機（N=50／k=1.5 緩衝／每 20 個交易日再平衡）。**再平衡觸發用交易日曆算距上次幾天，不用「跑了幾次」**——漏跑一天不會讓排程永遠差一天。**驗證＝獨立實作 replay 582 天對 `portfolio_lab`：年化 38.15% vs 38.15%（差 0.002pp）、換手 82.7% vs 82.7%、都是 30 次再平衡**。中途一度差 1.70pp，查出是換手定義不同（`portfolio_lab` 記 `buy_frac`、我記 `(buy+sell)/2`，差在首次建倉那次的 0.5 分攤到 30 次 = 1.67pp，與觀測值吻合）
  - **`V6/run_v62_daily.py`**：每日 orchestrator。**每日源必須有當日資料（容許 0 天）**——既有健檢容許 5 天，所以 margin 落後 1 天會顯示 ✓，正是它漏掉真問題的原因。已知停更且決定不修的三個源不檢查（避免永遠不消的假警報）。**特徵矩陣建一次、三個模型共用**（矩陣 3–5 分是成本大宗、GPU 前向每 arm 只要 1 秒 → 並行跑多模型的邊際成本≈0）。完整性隨當天持股一起落檔（事後補不回來）。Telegram 告警，未設 token 就印 log、不 raise
  - **`app/backend/routers/v62.py` + `app/frontend/src/pages/BreadthPortfolio.jsx`**：`/breadth/portfolio`，分頁名稱刻意叫**「持股組合（20 日）」**——把週期寫進名字，「今天的清單」才不會被讀成「今天要買的清單」。非再平衡日顯示「**今天不需要任何動作**」+ 倒數 + 漂移度（持股還有幾檔在模型 Top50），**刻意不顯示「今日選股」**（同一份訊號改每日再平衡回測是 −19.9%）
  - **三個模型並行**（`v2_kg_nomacro` 5d 頭／`head10d`／`head20d`）。**`{5d,20d}` 不需要訓練——`v6_short_H_h20.pt` 就是**（`head20d_ablation` 的產物，582 天已驗證，那顆頭對 `Alpha_20d` 的 IC 0.1388 是三顆頭最高）。checkpoint 收在 **`V6/checkpoints/`**（新目錄、已 gitignore；`V6/models/` 受保護不放）
- **★★★ 「資料修正後 checkpoint 會 OOD」實測為零 → 原訂的「重建矩陣＋全部重訓」取消（2026-08-05 深夜）**
  - **起因**：base matrix 建於 2026-07-30，而 `balance_sheet`/`cashflow`（08-04 MOPS）、`financials`/`revenue`（08-05）之後都被改寫，外加 08-04 的 FCF 兩層修正 → 逐欄比對證實**落差 100% 在 Group C**（`Free_Cash_Flow` ρ=**0.0911**、`EPS_Surprise` 0.2587、`ROE` 0.6892、`EPS` 0.8019、`Gross_Margin` 0.8973、`PER` 0.9420、`Book_Value` 0.9639），Group A/B 逐位元相同
  - **我當時的判斷是「checkpoint 在錯的 Group C 上訓練 → 修正後的資料是 OOD → 可能要重訓」。用現在的資料重評 582 天，實測推翻**：
  ...
  - → **每一項都在雜訊底線內**。而且那個缺陷是八個 arm **共用**的，依 2026-08-03 已建立的判準「共用的缺陷不影響相對比較」，八模型表的相對結論也不受影響
  - **連帶取消**：矩陣重建、`processed_v6.zip` 重新上傳（3 GB）、七個 Mamba arm 重訓（≈29h A100）。剩下的訓練需求全是**往前看的**（47 維變體），不是修補
- **★★ margin/daytrade 不是 T+1，是我們抓太早（2026-08-05，更正一條寫錯的記載）**。詳見上方「資料抓取稽核」那條的 ❌❌ 更正段。**解法是排程 19:35 → 21:30，不需 `shift(1)`、不需重訓**。順帶做了完整的抵達時間稽核：十個每日源裡八個在 19:35 就有當日資料，只有 `margin`/`daytrade` 沒有；`dividend` 落後 1 天是**事件驅動**（沒有公司公告）不是時間問題
- **★ EPS 截斷是局部缺陷、不是全歷史（2026-08-05）**。08-04 抓到「FinMind 把 16 支的 EPS 截成整數」後，擔心整個 EPS 歷史都被污染。實測「EPS 剛好是整數」的比例**逐年 2004→2026 全部落在 1.0%~1.8%**、2026Q1（MOPS 來源）1.37% 同水位 → **沒有系統性截斷**。那 16 列只是 FinMind 2026Q1 那批（僅 216 支）的局部缺陷，且已被 MOPS 覆蓋。**判別法：真實 EPS 幾乎不可能剛好是整數，所以「整數比例」不需要 ground truth 就能驗**
- **★★ GRU 補 purge + 換 v2 基礎 + 三個 seed：排名從第 2 掉到第 4，並量到 run-to-run σ（2026-08-05）**
  - **舊表的 GRU 是錯的**：`+31.3% / Sharpe 1.401` 是用 **v1 資料基礎（除權息還原之前）+ 完全沒有 purge** 跑出來的。修正後（v2 基礎 + purge=30、三個 seed）是 **19.6% / 0.996 平均 → −11.7pp**，排名 **第 2 → 第 4**
  - **★ run-to-run σ（本專案第一次量到）**：三個 seed 的 Top50 年化 21.2% / 21.1% / 16.5% → **σ = 2.68pp**、Sharpe σ = 0.126
  - **★★ 但 decile spread Sharpe 的 σ 只有 0.019**（2.395 / 2.383 / 2.421）——**穩定 40 倍**。機制很直觀：Top50 只押 50 檔、易被少數幾支主導；decile spread 用了全部約 1,900 支，個股雜訊互相抵銷。→ **比較模型優劣用 decile spread，不要用 Top50 年化**；而且**用 decile 就不需要每個模型都跑多 seed**
  - **一個結論部分存活**：「GRU 兩端訊號強」仍成立（decile 2.846 → **2.400 仍是全場第二**，只輸 `v2_kg_nomacro`），但「GRU 排第 2」不成立
  - **程式**：`baseline_rnn.py` 加 `--seed` / `--purge` / `--tag`（commit `d4b6cef`）。⚠️ purge 必須**同時套用在各 horizon 的訓練列**上——只改 `val_start` 不夠，被剔除的尾端日子會從 `val_rows` 溜回訓練迴圈
- **★ 「同一把尺」全面稽核（2026-08-05，使用者提出「不然比較性就消失了」後做的）**
  - purge ✅ 已統一（全部 30 交易日）／資料基礎 ✅ 全部 v2 本機矩陣／標籤窗口口徑 ✅ 一致
  - **panel 原本不一致**：Mamba 五組 1,047,772 列 / 1,913 支，ridge/gbdt/gru 1,055,417 列 / 1,915 支，**差 0.74%**（Mamba 需要更長歷史視窗）。已產 `{model}__common.parquet` 限制到共同 panel（1,047,635 列）並重掃 → **排名完全不變、最大變動 1.5pp（gbdt），在雜訊底線之內** → 差異不重要，但已修
  - **⚠️ 剩下唯一真正的破口：seed**。只有 GRU 有 3 個，**其他七個模型都是單次抽樣** → 年化那一欄每個數字都帶著看不見的 ±2.7pp。**這正是要改用 decile spread 的理由**
  - **定稿對照表（共同 panel、全部 purge，decile 為主要量尺）**：
    ...
    ⚠️ **decile 與年化的排序不一致**（gbdt decile 第 5 但年化最後、ridge decile 最後但年化第 4）——decile 量的是**全市場排序能力**，年化量的是 **Top50 這個特定用法**。兩者都要看，但**比較模型優劣用 decile**
- **★★ 標籤 horizon 的 Mamba 版跑完：20d 沒有勝出，而且這個實驗沒有能力回答這個問題（2026-08-05）**。完整紀錄 `docs/head20d-ablation-result-2026-08-05.md`：
  - **主結果（N=50/k=1.5/20 日）**：第二顆頭訓練在 10d = **+45.9%/Sharpe 1.853**、訓練在 20d = **+39.2%/1.721** → **20d 輸 6.7pp**，與 Ridge（+6.8pp）、GBDT（+10.6pp）方向相反
  - **★★ 但兩個 5d 頭（同標籤、同 seed、同架構、訊號幾乎相同）的組合層差了 6.2pp** → **效應與雜訊同量級**。而且第二顆頭那一對的訊號差異是**兩倍**（逐日 Spearman 0.9445 vs 0.9879、Top50 重疊 68% vs 84%），組合層差距卻幾乎一樣（6.7 vs 6.2pp）——**效應沒有隨訊號差異放大**
  - **跨 N 符號會翻轉**：N=10 效應 +36.7pp、N=25 +13.7、N=50 +6.7、**N=100 −0.5、N=224 −2.4**。效應大的 N=10/25 正是已證實為單窗過擬合的區段 → **跑前定死的規則「20d ≥ 控制 +2pp 且同向」未達標，且符號不一致**
  - **⚠️ 不可用訓練 log 的 `val_ic_10d` 下結論**：h20 那一欄實際是「對 `Alpha_20d` 的 IC」（換欄換的是值），h10 峰值 0.1221 對 10d、h20 峰值 0.1405 對 20d，**標的不同**；而 20 天前瞻報酬本身就比較好預測
  - **三層驗證全過**：① 配對乾淨（兩組 `best_val_ic_5d` 差 **0.00003**、峰值同為 ep7）② 本機重現 Colab（逐日相關 **1.0000**、最大單日差 0.0020/0.0025）③ 同 panel（1,047,772 列 / 582 天、參數 1,659,005，與 `v2_kg_nomacro` 相同）
  - **跑前修掉一個會讓實驗作廢的缺陷**（commit `aeeb134`）：`head20d_ablation` **完全沒有設 seed**（`SEED` 只寫進輸出 JSON），直接呼叫 `train_short_model`、繞過 `groupd_ablation._train_one_arm` 的 seed 重設 → 兩組會是髒配對，與 GAT 消融 A vs B 同一個坑
  - **結論**：**不要把上線規格改成 20d 標籤**——不是證明它較差，而是**沒有證據支持改動**，而改動有實際成本（重訓 + 推論端對齊）。Ridge/GBDT 的結果仍成立，但那是 **5d→20d** 的跨度，本輪是 **10d→20d**，而 5d→10d 那一步（+38.0%→+39.2%）可能已吃掉大部分好處
- **★★ 第一次量到組合層的雜訊底線：N=50 約 ±6pp 年化（2026-08-05）**。方法是免費的——**任何雙頭消融裡「兩組共用的那顆頭」天然就是完美的雜訊對照**。連帶要修正既有判讀：
  - `v2_kg_nomacro` 勝 `v2_kg` 的 **+12.0pp 約是雜訊的 2 倍** → 大概是真的，但沒有原本呈現的那麼確定
  - **八模型表裡小於 6pp 的差距不該當數**（例：`v3_kg` +26.8% vs `v2_kg` +26.0% 的 0.8pp）
  - **雜訊隨 N 增大而下降**（N=224 只有 0.8pp）→ 要比模型優劣用**大 N 比較可靠；小 N 的漂亮數字最不可信**
  - **⚠️ 我先前給過一個錯的估計（1.6pp）**：那是用 Colab `test_portfolio` 的舊口徑（Top50/5 日）算的，與比較用的口徑（k=1.5/20 日）不同，**差了約四倍**。**量雜訊必須用與比較完全相同的口徑**
...
- **★★ 資料抓取稽核：三個真正的缺口（2026-08-03 深夜，使用者要求「上線前趁早確認」）**：
  ...
  - **❌❌ 這一條原本寫錯了，2026-08-05 已實測更正 ❌❌**（原文：「`margin_raw` 是結構性 T+1……訓練端對 margin 有 1 天 look-ahead（margin@T 實際上 T+1 才公布）」）
    - **實測**：2026-08-05 **21:11** 直接打端點，`Date=2026-08-05` 的 margin **已經公布**（TWSE 1,064 + TPEX 800 檔、恆等式全過）；同一天 **19:35** 抓時回「尚未公布」。`daytrade` 完全相同。
    - → **margin@T 是 T 日晚間公布的，不是 T+1。訓練資料沒有 look-ahead**，日期標記一直是對的（log 可見 19:35 抓到的是 `margin 2026-08-04` 並以該日期存入）。
    - → **唯一的缺陷是推論端抓太早**（19:35），`_merge_margin` 的 `ffill()` 讓推論當天用到昨天的值 = train/serve 不對稱。**解法是把排程往後移，不是 `shift(1)`、不需重訓。**
    - **教訓**：「抓不到」與「還沒公布」與「T+1 才有」是三件事，不可從第一個推論到第三個。判別方法很便宜——**在不同時間點直接打端點一次就知道**
  - **❌ 缺口 1（最嚴重）：財報三源的覆蓋斷崖**。`financials_raw` 2025-12-31 有 **2,475 支** → 2026-03-31 只有 **176 支（7%）**；`balance_sheet_raw`／`cashflow_raw` 2026-03-31 只有 **16 支（0.7%）**，而且**完全不在任何每日流程裡**（`_catch_up_monthly` 只涵蓋 revenue + financials）。Q1 2026 法定申報期限是 5/15，現在 8 月。→ `fundamentals_v2=True`（部署 checklist 指定）要靠資產負債表算 `Book_Value`/`ROE`/`PBR`，實質上 99% 的股票會 ffill 到 2025-12-31 的值
  - **❌ 缺口 2：月營收覆蓋腰斬**。`revenue_raw` 2026-04 有 1,958 支 → 2026-05 起只有 **871 支（44%）**
  - **缺口 1/2 的共同根因**：FinMind 逐股滾動補齊（IP 封鎖後降到每天 80+40 支）**太慢**。實測速率 financials 的 2026-03 從 16 支（07-29）→ 176 支（08-03）＝ 約 **32 支/天**，補完 2,300 支還要約 70 天，而 Q2/Q3 持續進來 → **這個速率追不上**
  - **❌ 缺口 3：`trading_status_raw` 不在每日流程**（停在 2026-07-31，靠 `V6/experimental/fetch_trading_status.py` 手動跑）。組合建構系統的處置股限制要用它
  - **已知且已決定不修**：`macro_raw`／`fear_greed`／`business_indicator`（Group D 已證實負貢獻）。健檢的 3 項警告全屬此類
- **統一 purge + 三個靜默問題修復（2026-08-03 深夜）**：
  - **★ 統一 purge 後主對照表的排名有變**：稽核發現八模型表的隔離處理是**混的**——Mamba 四組有 30 個交易日（F6 的 `horizon=10 + embargo=20`）、Ridge/GBDT/GRU **0 天**。代價已量化（同 5d 標籤、同格）：**Ridge −0.5pp、GBDT −6.7pp**（+17.9% → +11.2%）→ **GBDT 從第 4 名掉到最後一名，落到 `no_gat` 之下**。→ **既有表格系統性偏袒 Ridge/GBDT/GRU，`v2_kg_nomacro` 的勝幅是被低估的**。已重出 `ridge__lab5d_p30`／`gbdt__lab5d_p30`。⚠️ **GRU 仍無隔離**（要重訓 1–2h 才能補），引用時必須標註
  - **附帶：purge 的影響在 20 天就飽和**（20 天與 30 天結果完全相同）→ 洩漏集中在最靠近切點的那段
  - **靜默問題 1 修復**：`_add_alpha_targets` 的 TWII fallback 原本完全無聲（名字叫 Alpha 卻不是 Alpha），加 `logger.warning` 明確指出退化。**只加警告不改值**，四個 Alpha 欄逐位元回歸 `max|Δ| = 0.000e+00`、NaN 型態相同
  - **靜默問題 2 修復（在源頭修）**：稽核出 `baseline_gbdt`／`baseline_ic_diagnosis`／`baseline_ridge_lasso`／`baseline_rnn` **四支用了協定依賴常數卻沒守門**，而 v1 的 `baseline_cache/baseline_base_59d.parquet` **真的存在**（1.46 GB、2026-07-12 建、在除權息還原之前）→ 忘了設 `MM_PROTOCOL=v2` 不會報錯，只會安靜地用舊資料。**修在 `baseline_common` 的 import 期橫幅**（印出解析到的協定/維度/矩陣路徑/建立時間，非 v2 時額外警告），所有消費端含未來新腳本自動受益。**刻意不 raise**——v1 仍是合法設定，問題是「看不見」不是「不能選」
  - **靜默問題 3 修復**：`label_10d.py` 加 `--check-only`，把漂移檢查變成可重複跑的工具（重用同一個 `verify()`，不另寫一份）。實測目前「協定窗內無漂移 ✅」
- **★★ 標籤 horizon vs 持有期：使用者的直覺是對的，而且效應比預期大（2026-08-03）**。完整紀錄 `docs/label-horizon-vs-holding-period-2026-08-03.md`。起因是使用者發現「20 天再平衡卻只預測 5 天」聽起來怪。**2 模型家族 × 3 標籤 × 5 頻率**，全部本機、不需 Colab：
  - **判讀規則跑之前定死並達標**（`label=20d` 在 20 日再平衡下 ≥ `label=5d` +2pp 且兩模型同向）：**Ridge +20.6% → +27.4%（+6.8pp、Sharpe 1.214 → 1.698）、GBDT +11.2% → +21.8%（+10.6pp）**。Mamba 換讀 10d 頭（不重訓）+38.0% → **+39.2%（Sharpe 1.713 → 1.838，全專案最高）**
  - **★ 機制不是「預測過期」，是「短標籤的排序比較跳」**：如果是前者，長標籤只該在匹配頻率上贏；實際上 **Ridge 的 20d 標籤在五個頻率上全部最好**（連 1 日再平衡都是）。看換手就清楚——20 日再平衡下 Ridge 換手 5d 標籤 81% → 20d 標籤 **56%**，成本 5.9% → 4.1%，而**毛報酬同時從 26.6% 升到 31.5%**。**長標籤同時改善兩端**：它獎勵「20 天內持續強勢」而不是「5 天內的短期彈跳」，名單日間變動小、不必一直追。→ 使用者的直覺指到真東西，但要重述成「**問題不在持有期超過預測期，而在短標籤製造了付不起的換手**」
  - **訊號層對齊確實有效（對角線佔優，三個家族一致）**：Ridge lab5d 對 Alpha_5d +0.0899 最高、lab20d 對 Alpha_20d +0.0924 最高；GBDT、Mamba 同型態。但幅度不大（Ridge 只有 +0.0079）→ **又一次「訊號層小差異在組合層放大」**（+0.008 IC → +6.8pp 年化）
  - **附帶最高值**：`v2_kg_nomacro` 的 **10d 頭對 Alpha_20d = +0.1311 / ICIR 1.490**，是本專案量過最高。且所有模型對 Alpha_20d 的 IC 都高於對 Alpha_5d → **20 天前瞻報酬本身比 5 天好預測**，獨立支持上面的機制
  - **⚠️ 必須先補 purge 才不會偏袒 20d**：現行協定 `TRAIN_END` 緊接 `TEST_START`、**沒有間隔**，5d 標籤洩漏 5 天、20d 洩漏 20 天 → 不處理的話 20d 白拿優勢。三組一律 `purge_days=20`。**連帶：本輪 Ridge-5d ≠ 既有 `ridge.parquet`，兩者不可混用**
  - **★ 大盤區間切分修正了一個既有結論的適用範圍**：§7c(⑩) 記「`v2_kg` 的優勢來自下跌段保護」——那對 `v2_kg` vs `no_gat` 成立，但 **`v2_kg_nomacro` vs `v2_kg` 完全相反**：上升段 +91.9% vs +58.2%（**+33.7pp**）、下跌段 **−24.2% vs −15.8%（差 8.4pp、更爛）**。→ **+38% 對多頭 regime 的曝險比 `v2_kg` 更高**，移除 Group D 似乎也拿掉了一部分下跌段防禦性 → 外部效度那個缺口更急迫
  - **驗證紀錄**（使用者要求「品質優先於速度」）：`Alpha_5d/20d` 重建 vs base matrix 協定窗內 5.47M 列 **max|Δ|=0**；`rank_5d/20d` 亦 **max|Δ|=0**（覆蓋數相同的日子）；`Alpha_10d` std 單調 0.0612 < 0.0898 < 0.1311（接近 √t）；相鄰 horizon 相關 0.715 / 0.705；`export_scores` 預設參數重跑 ridge **逐位元相同**；`load_xy(extra_labels=False)` 逐位元不變；Mamba 的 Group D 輸入探針雙向都對
  - **兩個過程發現**：① **`Alpha_Nd` 其實是原始前瞻報酬、沒有減大盤**（`_merge_macro` 的 rename 作用在 `m = df_macro.copy()` 上 → 呼叫端沒有 `TWII` 欄 → 走 else 分支）。實測 `max|Δ|=0`。**但影響是零**——標籤轉成每日 rank、IC 是 Spearman，減一個當日常數不改變任何結果 → **不修**，`label_10d.py` 刻意複製實際行為（修了反而讓 10d 與既有 5d/20d 不同義）② base matrix 建好後 `prices_raw` 有**一格歷史值**被除息還原改寫（2949 於 2026-07-29，59.0 → 56.969006），落在測試窗之外 → 閘門設計成「窗內零容忍、窗外據實列出」
  - **⚠️ Mamba 沒有真正的 20d 標籤組**：`ShortModelV6` 只有 `[head_5d, head_10d]` 兩顆頭，20d 必須重訓。腳本已備好（`head20d_ablation.py`，用 F5 的「換欄」技巧保持架構/RNG 完全相同，兩組約 7.2h）**尚未執行**。且 10d 頭與 5d 頭共用 trunk、early stopping 看的是 5d 頭 → Mamba 那兩列只能當方向性證據
  - **操作教訓**：四個背景任務同時被砍（與既有紀錄同一模式）。依既有教訓先確認殘留程序（Windows 無 python、WSL VM 已重啟）才重啟，改**前景分段**跑。另抓到一個靜默錯誤：WSL 指令沒帶 `MM_PROTOCOL=v2` → `baseline_common.CACHE_DIR` 解析成 v1 路徑 → 找不到標籤檔 → 退回佔位，表現成「vs Alpha_10d 與 vs Alpha_20d 的 IC 完全相同」。已改成不依賴環境變數的寫死路徑
- **★★ 2×2 最佳格過 portfolio_lab：訊號層新高換到了錢，八個模型全面最好（2026-08-03）**。分數 `V6/experimental/result/scores/v2_kg_nomacro.parquet`（1,047,772 列 / 582 天，WSL RTX 3060 前向 **7.2 分鐘**，不需 Colab、不需重訓）。同窗同格 **N=50 / k=1.5 / 20 日 / 無流動性過濾**：

  | model | 年化 | Sharpe | MDD | 換手 | 超額 | decile Sh | mean IC | ICIR |
  |---|---|---|---|---|---|---|---|---|
  | **v2_kg_nomacro** | **+38.0%** | **1.713** | −24.8% | 82% | **+19.9%** | **4.999** | **0.1139** | **1.332** |
  | gru | +31.3% | 1.401 | −25.9% | 86% | +13.9% | 2.846 | 0.1014 | 0.781 |
  | v3_kg | +26.8% | 1.522 | −20.0% | 76% | +9.4% | 1.928 | 0.0985 | 0.850 |
  | v2_kg | +26.0% | 1.487 | −20.2% | 75% | +8.8% | 1.905 | 0.0988 | 0.843 |
  | ridge | +21.2% | 1.231 | −24.8% | 81% | +4.5% | 1.064 | 0.0899 | 0.843 |
  | gbdt | +17.9% | 0.942 | −33.0% | 78% | +2.1% | 1.689 | 0.1030 | 0.836 |
  | old_kg | +16.6% | 1.036 | −20.8% | 72% | +0.5% | 1.231 | 0.0935 | 0.752 |
  | no_gat | +12.2% | 0.662 | −31.6% | 84% | −2.5% | 1.664 | 0.0867 | 0.717 |

  - **對 `v2_kg` 是 +12.0pp 年化、decile spread Sharpe 1.905 → 4.999（2.6×）**，而 IC 只多 +0.015。這是 GRU 診斷已量出的機制（**Spearman IC 對十分位輪廓的斜率不敏感**）的第二次實例，且幅度更大 → **「IC 小幅改善換不到錢」這個經驗法則要收窄**：它在同一模型家族內、輪廓形狀沒變時才成立
  - **四項可疑處檢查全過**（好結果先當可疑處理）：① **panel 與 `v2_kg` 的 (Date, stock_id) 逐對完全相同**（1,047,772 列、582 天 × 1,913 支）→ 這是乾淨配對、只有 score 不同（相關 0.785）② 逐日 IC 對 Colab **+0.1145 vs +0.1145、差 −0.0000、逐日相關 1.0000**、最大單日差 0.0018 ③ 前半 +35.6%(Sh 2.18) / 後半 **+40.5%(1.52)**，兩半都強（對照 ridge 29.5%→13.4%、gru 18.9%→45.1%）④ 第一個 batch 的 X 張量 Group D 12 個位置 **absmax = 0.000000**，對照組 `v2_kg` 是 3.000000 → **雙向都驗過**，推論輸入與訓練輸入一致
  - **★ 沒預期到的發現：拿掉 Group D，連 2026 年的 IC 衰退也一起消失**。滾動 60 日 IC 逐年 **2024 +0.1146 / 2025 +0.1156 / 2026 +0.1117**（幾乎持平），而 `v2_kg` 是 0.1047/0.1061/**0.081**、gru 0.0957/0.1164/**0.0907**。CLAUDE.md 原本把「2026 是 11 年最低」列為**要盯的訊號**——現在看起來那多半是 Group D 的過擬合在測試期後段失效，**不是訊號本身在退化**。這是 Group D 為負貢獻的第三條獨立證據（前兩條：Δ=−0.0186 的直接消融、Group D 對逐日 IC 注入雜訊讓 std(Δ) 大 2.9 倍）
  - **附帶佐證**：20 天小樣本時 `v2_kg` 對 Colab 的逐日相關是 0.9954、`v2_kg_nomacro` 是 **1.0000** → 本機/Colab 的二階差異（§4.4，`macro_norm="ts"` 是 expanding z-score、起點不同）**確實就住在 Group D**，歸零後自然消失
  - ⚠️ **交易限制的代價是全場最大，且是既有記載的例外**：擋漲跌停＋處置股後「標準因子研究口徑」+25.8% → **+20.3%（−5.5pp）**、「貼近實際操作」+49.3% → **+41.9%（−7.3pp）**。CLAUDE.md 原記「影響已量化（±1pp 內）」**在這個模型上不成立**（gru 在第二個 headline 也是 −7.3pp）。不過**扣完之後仍是最高**（次高 gru 12.9% / 26.7%）
  - ⚠️ **限制沒有改善**：單一 seed、單一 582 天多頭窗、**無 WF**（與其他 Mamba 組同一個限制）。全網格最佳格 N=10 是 +66.6%/Sharpe 1.97 **不可引用**——WF 已證實 N=10 是單窗過擬合。decile spread 含 short 腳，**long 腳單獨 +47.3%** 才是可落袋的部分
  - **程式改動（`V6/experimental/score_mamba_local.py`，純附加、既有 4 個 arm 行為不變）**：`ARMS` 改成 dataclass（含 ckpt 名／是否歸零 Group D／對照 JSON 來源）；新增 arm `v2_kg_nomacro`；Group D 歸零**直接 import `groupd_ablation.zeroed_macro`**（F5 紀律 ③：不另寫一份）且 **context 包住整個推論迴圈**（Dataset 是 lazy loading，提早還原會靜默吃到未歸零的 macro）；新增**第一個 batch 的 Group D absmax 探針**（與 arm 設定不符當場中止）；新增 `check_ref_windows()` 跑前確認對照窗與 val 窗是同一批日期；`--max-days` 改寫到 `{arm}__partialNd.parquet`——**舊版小樣本會直接覆蓋正式分數檔**，而 `portfolio_lab --sweep` 是 glob 整個 `scores/`，混進一個 20 天的檔會讓那個模型整份掃描變成錯的；驗證 JSON 改依 arm 合併（**舊版整份覆寫、已實際把 `v3_kg` 以外的紀錄清掉過**，本次從 git 救回）
  - **兩個環境事實（下次會再遇到）**：① checkpoint 不必手動下載——**Drive 桌面同步就掛在 `G:\我的雲端硬碟\MarketMamba_V6\checkpoints\`** ② **WSL 的 pandas 已升到 3.0.2 / numpy 2.4.3**，`portfolio_lab` 在那邊會炸（`to_numpy()` 回唯讀陣列 → `np.nan_to_num(copy=False)` 失敗）→ **組合層掃描一律在 Windows 端跑**（pandas 2.2.2，與既有九個模型同環境）；WSL 只負責 GPU 前向
...
### 進行中
- **V6.2 上線（週一 2026-08-10）**。程式已全部完成並 push（commit `03669d9` → `a9f6f1d`）。剩下三件，順序見上方 ▶ 區塊：① **V6.2 自帶資料抓取**（要退役 V6.1 就必須做，否則 V6.1 一停 V6.2 就沒有新資料）② 排程 21:30／22:15 ③ 第一天 `--first-day` 建倉
- **⚠️ V6.1／雙模型的退役時機**：使用者 2026-08-05 已同意拆掉，但**要等 V6.2 連續跑順幾天之後**。V6.2 目前仍依賴 V6.1 的 `run_daily_update()` 更新 raw parquet
- **Colab：47 維 arm**（`dim47_ablation`，2026-08-05 23:00 開跑、約 3.6h）。門檻 +0.009 寫死在腳本，未達標維持 59 維 + mask
...
- [x] ~~**★ 把 2×2 最佳格的 checkpoint 產成分數、過 portfolio_lab**~~ **✅ 已完成（2026-08-03）**：+38.0%/Sharpe 1.713，八個模型全面最好，見「最近完成」第一條。**F6 這條線至此完整收尾**
- **F6 下一階段的計畫待討論（2026-08-03 更新）**：這一輪（GAT 三組 + 2×2 + GRU 診斷 + 組合層驗證）**已完整收尾，沒有未完成的尾巴**。**下一輪做什麼尚未決定**，討論時可用的素材：
  1. **run-to-run σ 從沒量過**，而現在有**三個**大結論壓在單一 seed 上（GRU 兩端優勢、Group D 負貢獻、**`v2_kg_nomacro` 組合層 +12pp**）。最便宜的補法：GRU h64 用 2~3 個不同 seed 重跑（WSL、每個約 1~2 小時），一次同時得到 σ 與 GRU 的複驗。⚠️ 但那量的是 GRU 的 σ，**Mamba 的 σ 要另外用 Colab 重訓才有**（每個 seed 約 3.6h）
  2. **`v2_kg_nomacro` 的外部效度是現在最大的缺口**（2026-08-03 新增，優先度可能最高）：它是全場最好的模型，**證據卻只有單一 582 天多頭窗、單一 seed、無 WF**。Ridge/GBDT 有 11 年 WF、GRU 沒有、Mamba 全家都沒有。要做得付 Colab 重訓的錢（WF 每個 fold 重訓 → 很貴），或退而求其次用**大盤上升段/下跌段切分**（`portfolio_lab` 已有這個維度，零成本、今天就能算）
  3. **`INPUT_DIM` 59 → 47**（砍掉 Group D）要不要正式落規格 —— 證據又多了一條（2026 年 IC 衰退隨 Group D 一起消失）
  4. **GRU 的「兩端訊號更耐放」值得追**——它正好落在專案已確立最會賺錢的軸（低頻/省換手）上
  5. 多尺度主模型用 `epochs=15~20` 重跑（§3 那輪排程壞掉、數字不可用）
  6. 中性化用 Mamba 複驗（F5 給的是方向正但不顯著）
  7. **接班系統的設計**（scanner 退役後）—— 現在有了明確的候選規格：`v2_kg_nomacro` 分數 + N=50/k=1.5/20 日。缺口清單在 `docs/portfolio-construction-baseline-v1.md` §5b
...
> 判定門檻 **+0.009** 已寫死在腳本裡，未達標就**維持 59 維 + mask**（那是已被
> 組合層驗證過的設定；換架構要把推論端接線整套重做一次）。
>
> ### 上線前還沒做的三件事（按順序）
> 1. **V6.2 自帶資料抓取**（使用者 2026-08-05 決定要退役 V6.1）。
>    現在 `run_v62_daily.py` **不自己抓資料**，吃的是 V6.1 `run_daily_update()`
>    更新好的 raw parquet → V6.1 一停，V6.2 就沒有新資料。
>    做法：把 `run_daily_inference.py` 的資料更新段搬進 V6.2 的流程。
>    ⚠️ **在 V6.2 連續跑順幾天之前不要拆掉 V6.1／雙模型**。
> 2. **排程**（指令在下方「下一步」）：`PersonalOS_Daily` 19:30 → **21:30**、
>    新建 `MarketMamba_V62` **22:15**。⚠️ 新任務要勾「只在使用者登入時執行」
>    （WSL2 需要使用者 session，SYSTEM 帳號跑不動）。
> 3. **第一天用 `--first-day`** 強制建倉：`v62_daily.bat --first-day`
>
> ### 使用者提的 UX 問題（流程確認沒問題之後再做）
> - 推論進度視窗（tkinter/WSLg）**太醜、且不能顯示中文**
> - `.bat` 執行時會跳出**沒有意義的小黑窗**
>
> ### 📋 舊交接單 `docs/session-handoff-2026-08-05.md` 仍可讀，但它停在
> 「卡在推論端接線」那一步——**那一步已經做完了**，以本區塊為準。
>
...
> 這會改變八模型表裡哪些差距可以引用。詳見「最近完成」前兩條。
>
> 首跑（Colab CLI）失敗的紀錄留在 `docs/head20d-run-handoff-2026-08-04.md`：
...
- [x] ~~**★ WSL：把 2×2 最佳格產成分數過 portfolio_lab**~~ **✅ 已完成（2026-08-03）**：實測 7.2 分鐘，+38.0%/Sharpe 1.713 全場最好。答案是**有換到錢，而且放大了**（IC +0.015 → 年化 +12pp）
- [x] ~~大盤上升段/下跌段切分~~ **✅ 已完成（2026-08-03）**：`label_horizon_report.py` §C。結果不利於 `v2_kg_nomacro`——優勢**全在上升段**（+91.9% vs `v2_kg` +58.2%），下跌段**更差 8.4pp**（−24.2% vs −15.8%）
- [ ] **【優先級 1】`v2_kg_nomacro` 的外部效度**：它是全場最好的模型，但只有單一 582 天多頭窗，而上面的區間切分顯示它**對多頭 regime 的曝險比 `v2_kg` 更高** → 這個缺口比原先評估的更急迫。真 WF 要付 Colab 重訓的錢，先討論值不值得
- [x] ~~**缺口 3 的資料補齊**~~ **✅ 已完成（2026-08-03 深夜）**：`fetch_trading_status.py --years 2015 2026` 重建，補到 2026-08-03（+44 筆：處置 28／注意 16），**歷史零遺失**（舊有但新無 = 0 筆，跑前已備份）。⚠️ **剩「接進每日流程」未做**——現行 `build()` 是**整檔重建**不是增量，直接排進每日會每天重抓 11 年；需要先加增量路徑再接 `run_daily_update`
- [ ] **★ MOPS 整批端點可行性已驗證通過（2026-08-03 深夜，純讀未改程式）**，四個端點全部可用、**一季兩次請求取代 FinMind 的 ~1,900 次逐股查詢**：

  | 端點 | 內容 | 實測 |
  |---|---|---|
  | `ajax_t163sb05` | 資產負債表 | 上市 1,046 + 上櫃 883 = **1,929 家** |
  | `ajax_t163sb04` | 綜合損益表 | 同上（含 營業收入／營業成本） |
  | `ajax_t163sb20` | 現金流量表 | 1,046 家 |
  | `nas/t21/{sii,otc}/t21sc03_{年}_{月}_0.html` | 月營收 | 可用，**表格依產業分拆需合併** |

  - ⚠️ **host 必須是 `mopsov.twse.com.tw`**（`mops.twse.com.tw` 只回 686 bytes）——同 `data-source-implementation-traps.md` 那類陷阱
...
### 決策紀錄
- **★★★ 「資料修好了 → 舊結果都要重跑」不是自動成立的，要先量（2026-08-05）**：我提出「重建矩陣 + 全部重訓」，理由是 checkpoint 在錯的 Group C 上訓練。**實測代價是零**（年化 +0.1pp、decile +0.008、IC 0.0000，全在雜訊底線內）→ 計畫取消，省下 ≈29h A100 與 3 GB 上傳。**但這不與 2026-08-05 上午那條「資料基礎換過就必須重跑」矛盾**——GRU 那次是**資料基礎整個換掉**（v1→v2、除權息還原前後）掉了 11.7pp，這次是**十二分之一組特徵的修正**。判準應該是「**先花 15 分鐘量，再決定要不要花 29 小時**」，不是任一方向的教條
- **★★ 單日驗證要挑窗的中間，不要挑窗緣（2026-08-05，我自己踩到）**：挑 2026-06-02（窗最後一天）驗證，ρ=0.9398 未過；整窗 582 天的 median 是 0.9940、569 天過判準。窗緣**保證**受最新一次資料異動影響最大。**能跑整窗就跑整窗**——這次整窗只要 6.7 分鐘，比爭論單一天的數字便宜得多
- **★ 上線用的口徑必須逐行照抄回測，不可「順便改好一點」（2026-08-05）**：分數用 `eval()` 單次前向而非 MC-Dropout 平均、排序用 raw score 而非 SQ。兩者都「看起來更完整」，但都會讓實盤與 +38.0% 那個數字**不同義**。MC-Dropout 保留成顯示用的不確定性、預設關
- **★ 「抓不到」≠「還沒公布」≠「T+1 才有」（2026-08-05）**：CLAUDE.md 曾據此把 margin 記成結構性 T+1 並推導出「訓練端有 look-ahead」，進而規劃 `shift(1)` + 重訓。**實際只要在不同時間點打一次端點就能分辨**（19:35 沒有、21:11 有）。凡是要據此改動特徵語意的推論，先做那一次探測
- **★ 分頁名稱是防止誤用的第一道防線（2026-08-05 使用者提出）**：使用者說中間 19 天「沒顯示也沒關係，加個警告並把每日 Dashboard 的名字改掉就好」。→ 新頁叫「**持股組合（20 日）**」，把再平衡週期寫進名字本身，「今天的清單」就不會被讀成「今天要買的清單」。**刻意不做「今日選股」視圖**——回測證明每日再平衡是 −19.9%，做一個誘導該行為的介面等於把最賠錢的用法放在最顯眼的位置
- **★★ 比較模型優劣用 decile spread，不用 Top50 年化（2026-08-05）**：三個 GRU seed 實測，Top50 年化 σ = **2.68pp**、Sharpe σ = 0.126，但 **decile spread Sharpe σ 只有 0.019（穩定 40 倍）**。機制：Top50 只押 50 檔、易被少數幾支主導；decile spread 用全部約 1,900 支，個股雜訊互相抵銷。→ **連帶好處：用 decile 就不必每個模型都跑多 seed**（多 seed 很貴，Mamba 每個 seed 約 3.6h）。年化仍要看，但它回答的是「這個特定用法能落袋多少」，不是「哪個模型比較好」
- **★ 「同一把尺」要逐項稽核，不能只看最明顯的那一項（2026-08-05，使用者提出）**：purge 統一之後我以為表格可比了，實際查才發現 **panel 還差 0.74%**（Mamba 需要更長歷史視窗 → 少數股票日被剔掉）。已限制到共同 panel 重掃，**證實排名不變、最大變動 1.5pp**。→ 差異不重要，但**「小」要量過才能說**。完整清單：purge／資料基礎／panel／標籤／窗口／口徑／**seed**——最後一項至今只有 GRU 滿足
- **★ 資料基礎換過就必須重跑，不能沿用舊數字（2026-08-05，GRU 實例）**：`gru` 那筆 +31.3% 是 2026-07-14 用 **v1 快取 + 除權息還原之前**的資料訓的，卻一直放在八模型表裡與 v2 基礎的模型並列了一個月。修正後掉 **11.7pp、排名第 2 → 第 4**。→ **凡是資料層做過修復，所有既有結果都要標記為「待重跑」，而不是預設仍然可用**
...
- **★ 「IC 小幅改善換不到錢」這個經驗法則要收窄（2026-08-03，第二次被打臉）**：組合層槓桿比 IC 層大一個數量級的紀錄仍然成立（緩衝 +5.6pp、頻率 +7pp vs GAT +0.0107 只值 1–2pp），**但那是在同一模型家族、十分位輪廓形狀沒變的前提下**。`v2_kg` → `v2_kg_nomacro` 的 IC 只多 **+0.015**，年化卻多 **+12.0pp**、decile spread Sharpe 從 1.905 跳到 **4.999**。機制是 GRU 診斷已經量出的那一條——**Spearman IC 由整個分布主導、對兩端的推開程度不敏感**。→ 判讀新結果時，**IC 與 decile 輪廓要一起看**；只看 IC 的 Δ 會系統性低估「輪廓變陡」這一類的改進
- **★ 交易限制的代價不是固定的 ±1pp，會隨模型放大到 −7pp（2026-08-03，修正既有記載）**：CLAUDE.md 原記「接上處置股/注意股資料源，影響已量化（±1pp 內）」——那是在 Ridge/GBDT/`v2_kg` 上量的。`v2_kg_nomacro` 擋漲跌停＋處置後是 **−5.5pp / −7.3pp**（兩個 headline），gru 在第二個 headline 也是 −7.3pp。→ **模型越集中在「剛動起來」的股票，交易限制吃掉的越多**；新模型一律要重量這一項，不可沿用舊結論。**扣完之後的排序沒變**（`v2_kg_nomacro` 仍最高），所以是幅度修正、不是結論翻盤
- **小樣本模式不可以寫進正式輸出檔（2026-08-03，改掉一個既有的靜默地雷）**：`score_mamba_local.py` 的 `--max-days` 舊版直接覆寫 `{arm}.parquet`，而 `portfolio_lab --sweep` 是 **glob 整個 `scores/`**——一個 20 天的檔混進去，那個模型的整份掃描就是錯的，**而且不會報錯**。同一支腳本的驗證 JSON 也是整份覆寫、已實際把先前 arm 的紀錄清掉過（這次從 git 救回）。→ **凡是「快速驗證模式」的輸出，檔名就要與正式輸出分開**；凡是「一次跑一個 arm」的結果檔，寫入一律用 merge 不用覆寫
- **組合層掃描一律在 Windows 端跑（2026-08-03）**：WSL 的 pandas 已升到 3.0.2 / numpy 2.4.3，`portfolio_lab` 會炸在 `np.nan_to_num(ret_np, copy=False)`（pandas 3.0 的 `to_numpy()` 回唯讀陣列）。本機 Windows 是 pandas 2.2.2，**也是既有九個模型結果產出的環境** → 分工是 **WSL 只負責 GPU 前向、組合層數字全部在 Windows 算**，這樣新舊數字才在同一環境下可比
