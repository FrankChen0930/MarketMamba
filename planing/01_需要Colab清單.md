...
| # | 項目 | 狀態 | 優先級 | 備註 |
|---|---|---|---|---|
| 14 | 資料基礎升級計畫階段三-1（3b）：用新資料基礎（v2.0 協定）重訓一版 Mamba+GATv2 baseline | 待階段二（資料衛生修復＋序列輸出擴充）完成 | 中 | 見 `資料基礎升級計畫_baseline_common扶正.md` 階段三-1；不是要現在汰換 V6.1，只是驗證新基礎品質。**2026-07-23 範圍擴大**：使用者確認階段三不只是驗證 IC，而是要把特徵→模型→組合建構→風控整條梯子在新資料基礎上完整重跑一次當作品集素材；除了這項 Mamba 重訓，其餘子項目（Ridge/GBDT/GRU 重跑、組合建構方法比較、風控層設計）規模小或屬工程/分析性質，不需要 Colab，已同步列進 `02_不需要Colab清單.md` |
