...
# 🌌 終極願景：MarketMamba V5.0 (The Ultimate Alpha Engine)

在 V4.0 穩健運行並確立了純血 Mamba 在時序預測上的統治力後，本專案將啟動史上最大規模的重構計畫 —— **MarketMamba V5.0**。
V5.0 將不再只是一個預測模型，而是一個融合了「資料底層淨化」、「圖神經網路」與「動態博弈機制」的完整量化生態系。

## 1. 資料底層重構：除權息還原與大盤中性化
V5.0 將從根本上解決台股的兩大雜訊來源：
* **除權息陷阱 (The Ex-Dividend Trap) 修復：** 全面導入 `Adjusted Price` (還原權息股價)，消滅每年 6~8 月除息旺季造成的「假性跳水」，還原真實的技術指標與報酬率標籤。
* **大盤中性化 (Market Neutralization)：** 捨棄預測絕對報酬率，改為預測 **「超額報酬 (Alpha = 該股報酬 - TAIEX 大盤報酬)」**。剝離系統性風險 (Beta) 的雜訊，讓模型專注於尋找真正具備護城河的強勢股。

## 2. 真正的上帝視角：圖神經網路 (GNN) 實裝
V4.0 證明了「無限制的全連接 Attention」會導致模型被雜訊淹沒而坍塌。在 V5.0，我們將正式喚醒 `torch-geometric`：
* **產業關聯圖 (Adjacency Matrix)：** 透過 GNN 建立台股的社交網路。模型將受到「邊界限制」，例如台積電的 Attention 只能投射在「半導體產業鏈」或「高度相關 ETF」上。
* **動態板塊輪動：** 在清晰的圖結構下，Attention 層將不再平均化所有特徵，而是能敏銳地捕捉「資金從航運流向電子」的微觀板塊輪動訊號。

## 3. 損失函數的藝術：動態方向懲罰 (Curriculum Learning)
我們保留了極具價值的 `Directional-Aware Loss` (方向感知損失)，但在訓練機制上進行了史詩級升級：
* **溫水煮青蛙 (課程學習)：** 為了避免 V4.0 中「模型因過度恐懼而輸出安全常數」的悲劇，V5.0 將採用動態 Loss 權重。
* 在訓練初期 (Epoch 1-10) 採用純 MSE，鼓勵模型勇敢學習波動形狀；在模型建立自信後 (Epoch 11+)，系統會將做錯方向的懲罰權重從 `1.0` 緩步拉升至 `1.5` 甚至 `2.0`。在不引發模型坍塌的前提下，強迫提升實質交易勝率。

## 4. 決策最後一哩路：強化學習交易員 (RL Trading Agent)
