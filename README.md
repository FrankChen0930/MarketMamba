...
### 📊 MarketMamba V4.0 資料原物料盤點清單 (Data Inventory)

本專案採取 **「Hit and Run (打帶跑) 戰略」**：歷史 5 年資料庫依賴一個月的 FinMind VIP 權限進行光速建置；建置完成後，實盤推論階段將退回免費方案，採每日增量更新，確保系統無須長期訂閱付費資料，達成 0 元維運目標。

| 數據維度 (Dimension) | 具體特徵欄位 (Features) | 資料來源 (Source) | 根本是否免費? | 當前準備狀態 (Status) | 實盤更新策略 (Serving) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **1. 台股技術面** (量價結構) | `Open`, `High`, `Low`, `Close`, `Volume` | `yfinance` | ✅ 永久免費 | ⏳ 等待批量下載 | 每日免費 API 呼叫 |
| **2. 台股籌碼面** (三大法人) | `Foreign_Net_Buy` (外資買賣超)<br>`Trust_Net_Buy` (投信買賣超) | `FinMind` | ✅ 證交所公開資料<br>(受限 API 頻率) | ⏳ 等待 VIP 批量下載 | 每日免費 Token 緩速更新<br>(加 `time.sleep`) |
| **3. 衍生籌碼特徵** (自建特徵) | `Foreign_Impact_Ratio` (外資影響力) | 本地計算<br>(外資買賣超 / 成交量) | ✅ 本地算力 | ⏳ 隨籌碼面一併建置 | 本地即時計算 |
| **4. 美股宏觀維度** (大盤與情緒) | `US_SOX` (費半), `US_QQQ` (納斯達克)<br>`US_TNX` (美債), `US_VIX` (恐慌) | `yfinance` | ✅ 永久免費 | 🟢 已完成自動化 | 全市場共用，每日僅抓 1 次 |
| **5. 時序與防漏特徵** (時間工程) | `US_Market_Closed` (美股休市標記) | 本地計算<br>(As-Of Join 時間差) | ✅ 本地算力 | 🟢 邏輯已實作完成 | 本地自動比對日曆 |
