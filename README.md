...
為了進一步提升模型在真實市場的超額報酬 (Alpha)，下一個大版本將朝以下方向進化：

* [ ] **多模態特徵融合 (Multi-modal Inputs):** 整合「三大法人買賣超」、「融資券餘額」等籌碼面數據，捕捉技術面無法反映的深層資金動能。
* [ ] **動態圖學習 (Dynamic Graph Learning):** 捨棄靜態的 Pearson 相關係數，讓 GNN 的注意力機制 (Attention) 自主學習並預測每日變動的股票板塊連動權重。
* [ ] **方向感知損失函數 (Directional-Aware Loss):** 在模型訓練的 MSE 損失函數中加入懲罰項，針對「看錯方向」的預測給予更高權重，強迫模型提升勝率。
