...
**特徵維度**：56 維（個人實盤主線，不含相對強度特徵）／59 維（多模型並行線，多納入 RS_5d/RS_20d/RS_60d 相對強度特徵，總體環境維度以 mask 方式歸零——消融實驗證實總體環境因子在橫斷面排序上等同每日常數、拿掉後訊號品質反而顯著上升）。

### 每天實際在跑的不只一個模型

上面的圖是共用的基礎架構，但每天生產環境實際並行執行 **11 個模型設定（arm）**：8 種 Mamba 配置共用同一套 `MarketMambaV6`，差異只在知識圖譜版本、總體環境是否遮罩、GAT 是否啟用、預測頭訓練目標（5d／10d／20d）；另外 3 個是作為外部對照的傳統模型（Ridge、GBDT、GRU），走一套不遮罩總體環境的獨立特徵協定。再平衡頻率是「組合建構層」的參數、不是模型參數，所以這 11 份分數可以同時餵給 **19 個不同再平衡頻率的投資組合**，不需要為每個頻率重新訓練或重新推論。

---

## 每日自動化管線

兩條排程各自獨立執行，對應前面「廣度模型／高信念模型」的分野：

```
高信念線（V6.1，56 維主線 + 59 維雙模型對照）— Windows Task Scheduler → WSL2
  [1/7]  資料更新     yfinance + 交易所直連 + 財報直連
  [2/7]  特徵矩陣     建構因子矩陣 + 資料新鮮度檢查
  [3/7]  Mamba+GAT 推論 → df_kelly.csv, df_traj.csv
  [4/7]  LLM 市場報告  Claude API → market_summary.json
  [5/7]  歸檔         90 天滾動保留
  [6/7]  訊號掃描      進出場評分 → action_signals.json
  [7/7]  Push         git push → GitHub → 後端快取更新

廣度線（V6.2，59＋外部協定並行）— 高信念線之後接著跑，全程約 9 分鐘
  [1/6]  資料擷取     獨立抓取，不依賴高信念線的結果
  [2/6]  特徵矩陣     建一次、11 個模型 arm 共用
  [3/6]  多模型推論   8 份 Mamba 分數（先去重再前向）+ 3 份 Ridge/GBDT/GRU 分數（獨立 process）
  [4/6]  組合建構     11 份分數 × 19 個再平衡頻率的持股狀態機（純 CPU）
  [5/6]  前瞻績效彙總  逐日記錄實際報酬，與回測數字交叉驗證（獨立 process，失敗不影響持股紀錄）
  [6/6]  Push         git push → 後端快取刷新
```
