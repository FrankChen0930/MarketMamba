...
本專案採取 **「Hit and Run (打帶跑) 戰略」**：歷史 5 年資料庫依賴一個月的 VIP 權限進行光速建置 (榨乾伺服器)；建置完成後，實盤推論階段退回免費方案，採每日增量更新，達成 0 元實盤維運。

| 數據維度 (Dimension) | 具體特徵欄位 (Features) | 更新頻率 | 根本是否免費? | 戰略價值 (Alpha Source) |
| :--- | :--- | :--- | :--- | :--- |
| **1. 台股技術面** | `Open`, `High`, `Low`, `Close`, `Volume` | 每日 | ✅ 永久免費 | 捕捉市場價格與動能基準。 |
| **2. 法人籌碼面** | `Foreign_Buy` (外資), `Trust_Buy` (投信),<br>`Dealer_Buy` (自營商) | 每日 | ✅ 證交所公開 | 追蹤市場大資金的流向與控盤力道。 |
| **3. 散戶信用面** <br>*(✨新增榨乾目標)* | `Margin_Balance` (融資餘額),<br>`Short_Balance` (融券餘額) | 每日 | ✅ 證交所公開 | 衡量散戶情緒，捕捉「融資斷頭」或「軋空」行情。 |
| **4. 大戶流向面** <br>*(✨新增榨乾目標)* | `Whale_Hold_Ratio` (大戶持股比例),<br>`Retail_Hold_Ratio` (散戶持股比例) | 每週五 | ✅ 集保中心公開 | 洞悉籌碼集中度，抓出「千張大戶默默吃貨」的飆股。 |
| **5. 基本催化面** <br>*(✨新增榨乾目標)* | `Revenue_YoY` (營收年增率),<br>`Revenue_MoM` (營收月增率) | 每月 10 日 | ✅ 公開資訊觀測站 | 避免買到財報地雷，捕捉營收創高跳空行情。 |
| **6. 美股宏觀維度** | `US_SOX`, `US_QQQ`, `US_TNX`, `US_VIX` | 每日 | ✅ 永久免費 | 衡量全球資金水位與科技股風險偏好。 |
| **7. 時序與防漏** | `US_Market_Closed` (美股休市標記) | 每日 | ✅ 本地計算 | 根絕未來數據洩漏 (Lookahead Bias)。 |
| *(V5 預留區)* | *PTT/新聞 情緒分數 (FinBERT)* | *網路爬蟲* | *✅ 開源模型* | *⏸️ 暫緩，避免人為雜訊* | *未來實作* |
